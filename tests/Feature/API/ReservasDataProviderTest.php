...
class ReservasDataProviderTest extends TestCase
{
    use RefreshDatabase;
    use WithFaker;
        ...
        ->assertJson(fn(AssertableJson $json) =>
            $json
                ...
                ->has('categories.0.models.0',fn($json) =>
                    $json
                    ->where('name', $category_models[0]->name)
                    ->where('description', $category_models[0]->description)
                    ->where('default', (boolean) $category_models[0]->default)
                    ->where('image', asset("storage/{$category_models[0]->image}"))
                    ->etc()
                )
                ->has('categories.0.models.1',fn($json) =>
                    $json
                    ->where('name', $category_models[1]->name)
                    ->where('description', $category_models[1]->description)
                    ->where('default', (boolean) $category_models[1]->default)
                    ->where('image', asset("storage/{$category_models[1]->image}"))
                    ->etc()
                )
                ->etc()
        );
